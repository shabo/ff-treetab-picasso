...
`make build` now runs a gulp pipeline:

1. build unsigned ZIP into `dist/`
2. publish to AMO automatically if credentials are present

If `AMO_JWT_ISSUER` and `AMO_JWT_SECRET` are missing, publish is skipped and only the unsigned ZIP is produced.
...
Environment variables used by publish:

```sh
export AMO_JWT_ISSUER="..."
export AMO_JWT_SECRET="..."
# optional: listed (default) or unlisted
export AMO_CHANNEL="listed"
# optional: fail build when publish credentials are missing
export AMO_REQUIRE_PUBLISH="1"
```
